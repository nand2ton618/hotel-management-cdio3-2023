...
import { PrismaModelName } from '../interfaces'
...
const getOne = (modelName: PrismaModelName) =>
  catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!doc) return next(new HttpException('No data found', StatusCode.NotFound))
...
const updateOne = (modelName: PrismaModelName) =>
  catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!doc) return next(new HttpException('No data found with that ID', StatusCode.Forbidden))
